...
# ==========================================================
# 👑 ADMIN PANEL & FILE MANAGEMENT SECTION
# ==========================================================
...
async def manage_files_list(query):
    ...
    if not files:
        ...
        await query.edit_message_text("🗂️ Database khali hai. Koi file nahi mili.", reply_markup=InlineKeyboardMarkup(keyboard))
        return

    keyboard = []
    for f in files:
        keyboard.append([InlineKeyboardButton(f"📁 [{f['category']}] {f['file_name']}", callback_data=f"editfile_{f['_id']}")])
    
    keyboard.append([InlineKeyboardButton("🏠 Back to Admin", callback_data="admin_home")])
    await query.edit_message_text("📂 *Manage Files (Click on any file to Delete):*", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")

async def edit_file_options(query, file_id):
    file = material_col.find_one({"_id": ObjectId(file_id)})
    if not file: 
        await query.answer("❌ File nahi mili.")
        return
    
    keyboard = [
        [InlineKeyboardButton("🗑️ Delete This File", callback_data=f"del_{file_id}")],
        [InlineKeyboardButton("🔙 Back to List", callback_data="manage_files")]
    ]
    await query.edit_message_text(
        f"📝 *Managing File:*\n\n📄 Name: `{file['file_name']}`\n📂 Category: `{file['category']}`\n\nAap is file ke sath kya karna chahte hain?", 
        reply_markup=InlineKeyboardMarkup(keyboard), 
        parse_mode="Markdown"
    )
...
async def button_click(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "verify":
        ...
        material_col.delete_one({"_id": ObjectId(file_id)})
        await query.answer("✅ File Successfully Delete Ho Gayi!")
        await manage_files_list(query)
